...
fn main() {
    let red_cubes_target = 12;
    let green_cubes_target = 13;
    let blue_cubes_target = 14;
    let mut sum = 0;
    ...
    for game_line in file.into_inner() {
        match game_line.as_rule() {
            Rule::game => {
                ...
                if game.draws.iter().all(|d| {
                    d.red <= red_cubes_target
                        && d.green <= green_cubes_target
                        && d.blue <= blue_cubes_target
                }) {
                    sum += game.id;
                }
            }
